class Prank
	...
	def initialize(to_name, to_num, from_name, from_num)
		...
		# KH: since this logic is consistent and repeted in any given instance
		# --	of the Prank class, it should go in the initialize method.
...
# create custom message function
	...
	def call
    	...
    	:from => @from_num, # KH: need the @ to refer to the instance variable.
    	:to => @to_num,
    	:url => twimlet)
...
# pre-populated pranks
	...
	def so_fat
		...
		# KH: since @client is defined in the initialize method you can just call on it here.
		@client.account.calls.create(
    	:from => @from_num,
    	:to => @to_num,
    	:url => @url)
